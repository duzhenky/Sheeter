...
def read_table(path_to_file, file_extension):
    try:
        ...
        if file_extension == 'xlsx':
            ...
            df = None
            print(f'Файл с расширением {file_extension} не может быть разделён')
        return df
    except FileNotFoundError as e:
...
def separate_table():
    ...
    :param prefix: добавление некого префикса к наименованиям
    :param postfix: добавление некого префикса к наименованиям
    """

    path_to_main_file = input('Путь к файлу для разъединения: ')
    col_name = input('Наименование столбца: ')
    ...
    # создаём папку по названию файла
    name_folder = path_to_main_file.split('/')[-1].split('.')[0]
...
if __name__ == '__main__':
    while True:
        user_choice = input('\n\n1. Разделить таблицу;\n'
                            '2. Склить таблицы\n'
                            'Для выхода введите любую клавишу\n'
                            '\nВыберете действие: ')
